...
from novel_agent.runtime.judge_packet import compact_judge_issues
...
def test_compact_judge_issues_omits_in_draft_quotes_keeps_claim_id() -> None:
    ...
    compacted = compact_judge_issues(issues, draft)
    ...
    assert compacted[0]["scene_ids"] == ["v1c001_s1"]
...
def test_compact_judge_issues_keeps_quotes_absent_from_draft() -> None:
    ...
    compacted = compact_judge_issues(issues, "茶楼里灯火通明。")
...
def test_compact_judge_issues_aggressive_drops_all_quotes() -> None:
    issues = [
        {
            "issue_id": "issue_3",
            "claim": "开场信息越权",
    ...
    compacted = compact_judge_issues(issues, "茶楼里灯火通明。", drop_all_quotes=True)
    assert compacted[0]["evidence"][0]["quote"] == "见正文"
    assert compacted[0]["claim"] == "开场信息越权"
